...
@pytest.mark.parametrize("stem", ["bravo1", "maybe-delta__c07", "random"])
...
def test_weak_matches_cannot_reach_the_bank_unaided():
    ...
    assert parse_name("maybe-kilometer__c04__2syl__F#3__2.75-3.11") is None


@pytest.mark.parametrize("heard,expected", [
    ("Bravo!", "bravo"),
    ("bravo", "bravo"),
    ("bravu", "bravo"),        # recogniser spelling drift
    ("tango", "tango"),
    ("deltas", "delta"),       # an inflected form
    ("Calculator,", "calculator"),
    ("kilometer", "kilometer"),
    ("kilo", "kilometer"),    # clean prefix of a long word
])
